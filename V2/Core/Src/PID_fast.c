...
void PID_REG(Motor_Sruct *Motor){
  ...
  regD1=(Error1-Motor->Error_old1)*Motor->D_position*100; //becouse dt is 0.01
  if (regD1<(-10000))
  {
    regD1=(-10000);
  }
  ...
  if (Motor->I_velocity){
    Motor->regI2+=(Error2*Motor->I_velocity)/100; //becouse time is 0.01  error*Ki*dt
  }else{
    Motor->regI2 =0; 
  }
  regD2=(Error2-Motor->Error_old2)*Motor->D_velocity*100; //becouse dt is 0.01
  if (regD2<(-10000))
  {
    regD2=(-10000);
  }
