...
extern Motor_Sruct M1,M2;
extern uint8_t tic_count; 
extern uint16_t Step1_cnt_from_EXTI; 
extern uint16_t Step2_cnt_from_EXTI; 
/* USER CODE END EV */
...
//  }else{
    ...
    {
        motor->velocity=0;
    }else if(temp_counter>motor->last_counter_value)
    {
      if(__HAL_TIM_IS_TIM_COUNTING_DOWN(htim))
      {
        motor->velocity=-motor->last_counter_value-(__HAL_TIM_GET_AUTORELOAD(htim)-temp_counter);
      }else{
        motor->velocity=temp_counter-motor->last_counter_value;
      }
    } else{
      if(__HAL_TIM_IS_TIM_COUNTING_DOWN(htim))
      {
